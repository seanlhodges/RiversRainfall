...
###Building dataframes with data
There is a step missing here that implements the GetDataAvailability hack. The next chunk of code will not work without it. I will resolve this tonight.


```{r GetData}

source("functions.R")
ds <- rcData(ds,measurements)
# Just select sites that have  flow data
df_flow <- subset(ds,Flow == "OK")
...
for(i in 1:length(df_flow[ ,1])){
    ...
    # Handle the exceptions with the SOS call has no data to return
    if(xmlName(xmltop)!="ExceptionReport"){
        # Where there is no error, GetData
